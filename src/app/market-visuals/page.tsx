...
const TABS: Array<{
...
}> = [
  {
    id: "overview",
    label: "Overview",
    helper: "Live market cockpit",
    icon: Gauge,
  },
  {
    id: "chart",
    label: "Live Chart",
    helper: "Alpha Vantage OHLCV",
    icon: LineChartIcon,
  },
  {
    id: "forecast",
    label: "Forecast",
    helper: "Probability bands",
    icon: BrainCircuit,
  },
  {
    id: "technicals",
    label: "Technicals",
    helper: "Indicators and signals",
    icon: Sigma,
  },
  {
    id: "fundamentals",
    label: "Fundamentals",
    helper: "Company and earnings",
    icon: BookOpenText,
  },
  {
    id: "news",
    label: "News",
    helper: "Live sentiment feed",
    icon: Newspaper,
  },
  {
    id: "options",
    label: "Options",
    helper: "Realtime chain",
    icon: Target,
  },
  {
    id: "compare",
    label: "Compare",
    helper: "Relative performance",
    icon: BarChart3,
  },
  {
    id: "pine",
    label: "Pine Lab",
    helper: "OpenAI Pine v6",
    icon: Code2,
  },
  {
    id: "data",
    label: "Data Quality",
    helper: "Sources and candles",
    icon: Database,
  },
];
...
/*
 * Theme-aware forecast colors.
 *
 * These read from the global Slice color system but include fallbacks so the
 * chart remains visible if it is rendered outside the normal theme wrapper.
 */
const FORECAST_CHART_COLORS = {
  history:
    "var(--slice-heading, #0f172a)",
  base:
    "var(--slice-violet-text, #7c3aed)",
  baseEnd:
    "var(--slice-cyan-text, #0e7490)",
  bull:
    "var(--slice-green-text, #047857)",
  bear:
    "var(--slice-rose-text, #be123c)",
  upper:
    "var(--slice-cyan-text, #0e7490)",
  lower:
    "var(--slice-rose-text, #be123c)",
  grid:
    "var(--slice-border, rgba(100, 116, 139, 0.24))",
  axis:
    "var(--slice-muted, #64748b)",
  axisStrong:
    "var(--slice-border-strong, rgba(100, 116, 139, 0.42))",
  pointBorder:
    "var(--slice-surface-strong, #ffffff)",
} as const;
...
function ForecastLegend() {
  return (
    <div className="flex flex-wrap items-center gap-x-5 gap-y-3 border-b border-white/10 px-5 py-4 md:px-6">
      <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.12em] text-slate-400">
        <span
          className="h-[3px] w-8 rounded-full"
          style={{
            background:
              FORECAST_CHART_COLORS.history,
          }}
        />
        History
      </div>
      ...
      <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.12em] text-slate-400">
        <span
          ...
          style={{
            backgroundImage: `linear-gradient(90deg, ${FORECAST_CHART_COLORS.base}, ${FORECAST_CHART_COLORS.baseEnd})`,
          }}
        />
        Base forecast
      </div>
      ...
      <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.12em] text-slate-400">
        <span
          className="h-[3px] w-8 rounded-full"
          style={{
            background: `repeating-linear-gradient(90deg, ${FORECAST_CHART_COLORS.bull} 0 7px, transparent 7px 11px)`,
          }}
        />
        Bull scenario
      </div>
      ...
      <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.12em] text-slate-400">
        <span
          className="h-[3px] w-8 rounded-full"
          style={{
            background: `repeating-linear-gradient(90deg, ${FORECAST_CHART_COLORS.bear} 0 7px, transparent 7px 11px)`,
          }}
        />
        Bear scenario
      </div>
      ...
      <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.12em] text-slate-400">
        <span className="flex h-3 w-8 overflow-hidden rounded-sm border border-white/10">
          <span
            className="h-full flex-1"
            style={{
              background:
                FORECAST_CHART_COLORS.upper,
              opacity: 0.24,
            }}
          />
          <span
...
export default function MarketVisualsPage() {
  ...
  const [pinePrompt, setPinePrompt] =
    useState(
  ...
  const [pineType, setPineType] =
    useState<
      "indicator" | "strategy"
    >("indicator");
  const [pineCode, setPineCode] =
    useState(defaultPine("NVDA"));
  const [pineLoading, setPineLoading] =
    useState(false);
  const [pineWarnings, setPineWarnings] =
    useState<string[]>([]);
  const [pineProjects, setPineProjects] =
    useState<PineProject[]>([]);
  ...
  async function requestVisuals(
    ...
    const response = await fetch(
      `/api/market-visuals?${params.toString()}`,
      {
        cache: "no-store",
      },
    );
    ...
    try {
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : "Unable to load Alpha Vantage market data.",
      );
    } finally {
      setLoading(false);
    }
  ...
  /*
   * The final historical close is also used as the first point of every
   * forecast series. This gives the visual projection one common, accurate
   * origin instead of allowing the projected path to look detached from the
  ...
  const normalizedCompare = useMemo(
    () => {
      ...
      const primary =
        data.candles.slice(-180);
      const secondary =
        compareData.candles.slice(
          -180,
        );
      ...
      const a =
        primary.slice(-length);
      const b =
        secondary.slice(-length);
      const aBase =
        a[0]?.close || 1;
      const bBase =
        b[0]?.close || 1;
  ...
  async function generatePine() {
    ...
    try {
      ...
      setMessage(
        `Pine Script v6 generated with ${result.model}.`,
      );
    } catch (error) {
      setMessage(
        error instanceof Error
  ...
  function savePine() {
    const project: PineProject = {
      id: `pine-${Date.now()}`,
      name: `${symbol.toUpperCase()} ${pineType} ${new Date().toLocaleDateString(
        "en-US",
      )}`,
    ...
    setPineProjects((current) =>
      [project, ...current].slice(
        0,
        40,
      ),
    );
    ...
    <main className="relative min-h-screen overflow-hidden bg-[#050505] px-4 py-5 text-white md:px-6 lg:px-8">
      ...
      <div className="relative mx-auto grid max-w-[1900px] gap-5">
        <header className="rounded-[2rem] border border-white/10 bg-black/72 p-5 shadow-2xl shadow-emerald-950/25 backdrop-blur-xl md:p-7">
          <div className="flex min-w-0 flex-col gap-5 2xl:flex-row 2xl:items-start 2xl:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-3 max-w-5xl text-sm font-medium leading-7 text-slate-400 md:text-base">
                ...
                options, comparisons, and
                quantitative forecasts all
                originate from the
                configured Alpha Vantage
                account. Pine Lab uses the
                server-side OpenAI API to
                create Pine Script v6.
              </p>
            </div>
          ...
          <form
            onSubmit={submit}
            className="mt-5 grid min-w-0 gap-3 md:grid-cols-2 xl:grid-cols-[150px_150px_130px_130px_180px_auto_auto]"
          >
            ...
            <button
              disabled={loading}
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-600 px-5 py-3 text-sm font-black text-white shadow-lg shadow-emerald-950/30 hover:bg-emerald-500 disabled:opacity-40"
            >
              {loading ? (
          ...
          <Panel className="grid min-h-[600px] place-items-center p-8 text-center">
            <div>
              ...
              <h2 className="mt-4 text-2xl font-black">
                Loading Alpha Vantage
                market data
              </h2>

              <p className="mt-2 text-sm text-slate-500">
                Retrieving entitled
                quotes, OHLCV history,
          ...
          <>
                      ...
                      >
                        ...
                        >
                          <defs>
                            ...
                            >
                              ...
                              <stop
                                offset="100%"
                                stopColor={
                                  FORECAST_CHART_COLORS.baseEnd
                                }
                              />
                            </linearGradient>

                            <linearGradient
                              id="slice-market-forecast-upper-fill"
                              x1="0"
                              y1="0"
                              x2="0"
                              y2="1"
                            ...
                            <linearGradient
                              id="slice-market-forecast-lower-fill"
                              x1="0"
                              y1="0"
                              x2="0"
                              y2="1"
                          ...
                          <XAxis
                            ...
                            minTickGap={18}
                            stroke={
                              FORECAST_CHART_COLORS.axisStrong
                            }
                            tick={{
                              fill: FORECAST_CHART_COLORS.axis,
                              fontSize: 11,
                              fontWeight: 700,
                            }}
                            tickLine={{
                          ...
                          <Area
                            ...
                            strokeDasharray="3 4"
                            fill="url(#slice-market-forecast-upper-fill)"
                            fillOpacity={1}
                            dot={false}
                            connectNulls
                            isAnimationActive={
                          ...
                          <Area
                            ...
                            strokeDasharray="3 4"
                            fill="url(#slice-market-forecast-lower-fill)"
                            fillOpacity={1}
                            dot={false}
                            connectNulls
                            isAnimationActive={
                          ...
                          <Line
                            ...
                            activeDot={{
                              r: 4,
                              fill: FORECAST_CHART_COLORS.history,
                              stroke:
                                FORECAST_CHART_COLORS.pointBorder,
                              strokeWidth: 2,
                            }}
                            connectNulls
                            isAnimationActive={
                          ...
                          {/*
                           * A wider translucent line sits behind the main
                           * forecast. This creates a restrained glow without
                           * relying on an SVG filter that could cause browser
                           * or TypeScript compatibility issues.
                           */}
                          <Line
                            type="monotone"
                            dataKey="projected"
                            name="Forecast emphasis"
                            stroke="url(#slice-market-forecast-base)"
                            strokeWidth={9}
                            strokeOpacity={
                              0.16
                            }
                          ...
                          <Line
                            ...
                            activeDot={{
                              r: 6,
                              fill: FORECAST_CHART_COLORS.base,
                              stroke:
                                FORECAST_CHART_COLORS.pointBorder,
                              strokeWidth: 2.5,
                            }}
                          ...
                          <Line
                            ...
                            activeDot={{
                              r: 4,
                              fill: FORECAST_CHART_COLORS.bull,
                              stroke:
                                FORECAST_CHART_COLORS.pointBorder,
                              strokeWidth: 2,
                            }}
                          ...
                          <Line
                            ...
                            activeDot={{
                              r: 4,
                              fill: FORECAST_CHART_COLORS.bear,
                              stroke:
                                FORECAST_CHART_COLORS.pointBorder,
                              strokeWidth: 2,
                            }}
                  ...
                  <Panel>
                    ...
                    <div className="grid gap-3 p-5 sm:grid-cols-2 2xl:grid-cols-1">
                      <Metric
                        ...
                        helper={`${data.forecast.horizon} forecast steps`}
                      />
                      ...
                      <Metric
                        ...
                        helper={`${data.forecast.confidenceLevel}% band`}
                      />
                      ...
                      <Metric
                        ...
                        helper="From Alpha Vantage returns"
                      />

                      <Metric
                        label="Directional backtest"
                        value={percent(
                          data.forecast
                        ...
                        helper="Recent one-step history"
                      />

                      <Metric
                        label="Drift per step"
                        value={percent(
                          data.forecast
                            .driftPerStepPct,
                          4,
                        )}
                        helper="EWMA estimate"
                      />
                    </div>
                  </Panel>
            ...
            {view === "technicals" ? (
              <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(380px,0.8fr)]">
                <Panel>
                  <SectionHeader
                    eyebrow="Technical score stack"
                    title="Indicators computed from Alpha Vantage candles"
                    description={
                ...
                <Panel>
                  <SectionHeader
                    eyebrow="Latest indicators"
                    title={`${data.symbol} technical state`}
                  />
                  ...
                  <div className="grid gap-3 p-5 sm:grid-cols-2">
                    ...
                    <Metric
                      ...
                      value={percent(
                        latest?.cumulativeReturnPct,
                      )}
                      helper={`${candles.length} displayed candles`}
                    />
                  </div>
                </Panel>
              </div>
            ...
            "fundamentals" ? (
              <div className="grid min-w-0 gap-5 2xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
                <Panel>
                  <SectionHeader
                    eyebrow="Alpha Vantage OVERVIEW"
                    title={
                      data.company.name ||
                      data.symbol
                    }
                    description={
                      data.company
                        .description ||
                      "Company overview is unavailable from Alpha Vantage."
                    }
                    action={
                      <Badge
                        value={
                ...
                <Panel>
                  <SectionHeader
                    eyebrow="Alpha Vantage EARNINGS"
                    title="Reported EPS and surprises"
                    action={
                      <Badge
                        value={
                          data.earnings
                  ...
                  <div className="overflow-x-auto px-5 pb-5">
                    <table className="w-full min-w-[650px] text-left text-xs">
                      ...
                      <tbody>
                        {data.earnings.quarterly
                          .slice(0, 8)
                          .map((row) => (
                            <tr
                              key={`${row.fiscalDateEnding}-${row.reportedDate}`}
            ...
            {view === "news" ? (
              <Panel>
                <SectionHeader
                  eyebrow="Alpha Vantage NEWS_SENTIMENT"
                  title={`${data.symbol} live news and sentiment`}
                  description="Article titles, timestamps, relevance, and ticker sentiment are returned by Alpha Vantage; SLICE does not invent missing articles."
                  action={
                    <Badge
                      value={
                        data.news
                ...
                <div className="grid gap-3 p-5 lg:grid-cols-2 2xl:grid-cols-3">
                  {data.news.articles.map(
                    (article) => (
                      ...
                      >
                        ...
                        <h3 className="mt-3 break-words text-base font-black leading-6">
                          {article.title}
                        </h3>
                          ...
                          {article.topics.map(
                            (topic) => (
                              <span
                                ...
                                className="rounded-md bg-white/[0.055] px-2 py-1 text-[9px] font-black text-slate-400"
                              >
                                {topic}
                              </span>
                            ),
                        ...
                        {article.url ? (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="mt-4 inline-flex text-xs font-black text-cyan-300 hover:text-cyan-200"
                          >
                            Open source
                            article →
                          </a>
                  ...
                  {!data.news.articles
                    .length ? (
                    <div className="rounded-2xl border border-dashed border-white/10 p-10 text-center text-sm font-bold text-slate-500 lg:col-span-2 2xl:col-span-3">
                      Alpha Vantage did not
                      return matching news.
                    </div>
                  ) : null}
                </div>
              </Panel>
            ...
            {view === "options" ? (
              <Panel>
                <SectionHeader
                  eyebrow="Alpha Vantage REALTIME_OPTIONS"
                  title={`${data.symbol} real-time options chain`}
                  description="Real-time options require an Alpha Vantage plan that includes the REALTIME_OPTIONS endpoint. If the plan is not entitled, SLICE shows the provider error rather than simulated contracts."
                  action={
                    <Badge
                      value={
                        data.options
                    ...
                    >
                      {data.options
                        .available
                        ? `${data.options.contractCount} contracts`
                        : "Not available"}
                    </Badge>
                  }
                />
                ...
                {data.options.available ? (
                  <>
                    ...
                    <div className="max-h-[650px] overflow-auto px-5 pb-5">
                      <table className="w-full min-w-[1200px] text-left text-xs">
                        <thead className="sticky top-0 bg-zinc-950 text-slate-500">
                          <tr>
                            {[
                              "Contract",
                              "Type",
                  ...
                  <div className="m-5 rounded-2xl border border-emerald-400/25 bg-emerald-400/10 p-6 text-sm leading-7 text-emerald-100">
                    ...
                    <p className="mt-2">
                      {data.options.error ||
                        "The Alpha Vantage response did not contain an options chain. Confirm that the paid plan includes REALTIME_OPTIONS and Greeks access."}
                    </p>
                  </div>
                )}
              </Panel>
            ...
            {view === "compare" ? (
              <Panel>
                <SectionHeader
                  ...
                  title={`${data.symbol} versus ${compareSymbol.toUpperCase()}`}
                  description="Both series are independently requested from Alpha Vantage and normalized to a common starting value."
                  action={
                    <form
                      onSubmit={(
                        event,
                    ...
                    >
                      ...
                      >
                        ...
                        <Line
                          ...
                          name={`${compareSymbol.toUpperCase()} return %`}
                          stroke="#06b6d4"
                          dot={false}
                          strokeWidth={3}
                        />
            ...
            {view === "pine" ? (
              <div className="grid min-w-0 gap-5 2xl:grid-cols-[420px_minmax(0,1fr)_360px]">
                <Panel className="h-fit 2xl:sticky 2xl:top-5">
                  <SectionHeader
                    eyebrow="OpenAI Pine Lab"
                    title="Generate Pine Script v6"
                    action={
                      <Badge
                        value={
                          data.pineLab
                  ...
                  <div className="grid gap-4 p-5">
                    ...
                    <textarea
                      ...
                      className={cx(
                        INPUT,
                        "min-h-[220px] leading-6",
                      )}
                      placeholder="Describe the Pine indicator or strategy..."
                    />

                    <div className="rounded-2xl border border-cyan-400/20 bg-cyan-400/10 p-4 text-xs leading-6 text-cyan-100">
                      <div className="font-black">
                        Live context sent to
                        OpenAI
                      </div>
                    ...
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-2 rounded-xl bg-violet-600 px-4 py-4 text-sm font-black text-white hover:bg-violet-500 disabled:opacity-40"
                    >
                      {pineLoading ? (
                        <Loader2 className="h-4 w-4 animate-spin" />
                      ) : (
                ...
                <Panel>
                  <SectionHeader
                    eyebrow="Pine editor"
                    title={`${symbol.toUpperCase()} ${pineType}`}
                    description="Generated code uses TradingView Pine Script v6. Paste it into TradingView Pine Editor and compile before use."
                  />
                  ...
                  <div className="p-5">
                    <textarea
                      ...
                      spellCheck={false}
                      className="min-h-[780px] w-full resize-y rounded-2xl border border-white/10 bg-black/70 p-5 font-mono text-xs leading-6 text-emerald-100 outline-none ring-violet-500 focus:ring-2"
                    />
                  </div>

                  {pineWarnings.length ? (
                    <div className="mx-5 mb-5 rounded-2xl border border-amber-400/20 bg-amber-400/10 p-4 text-xs leading-6 text-amber-100">
                      {pineWarnings.map(
                        (warning) => (
                          <div
                            key={
                        ...
                        >
                          ...
                          <div className="mt-3 grid grid-cols-2 gap-2">
                            ...
                            <button
                              ...
                              className="inline-flex items-center justify-center rounded-xl border border-emerald-400/20 bg-emerald-400/10 px-3 py-2 text-xs font-black text-emerald-100"
                            >
                              Delete
                            </button>
                          </div>
            ...
            {view === "data" ? (
              <div className="grid min-w-0 gap-5">
                <Panel>
                  <SectionHeader
                    eyebrow="Provider integrity"
                    title="Every requested source and its status"
                    description="Unavailable endpoints remain unavailable; SLICE does not replace them with demo values."
                    action={
                      <Badge
                        value={
                          data.quality
                    ...
                    {data.quality.calls.map(
                      (call) => (
                        ...
                        >
                          ...
                          {call.error ? (
                            <p className="mt-3 break-words text-xs leading-5 text-emerald-200">
                              {
                                call.error
                              }
                            </p>
                  ...
                  {data.quality.warnings
                    .length ? (
                    <div className="mx-5 mb-5 rounded-2xl border border-amber-400/20 bg-amber-400/10 p-4 text-sm leading-7 text-amber-100">
                      {data.quality.warnings.map(
                        (warning) => (
                          <div
                            key={
                              warning
                ...
                <Panel>
                  <SectionHeader
                    eyebrow="Alpha Vantage candle table"
                    title={`${latestRows.length} latest enriched observations`}
                    description="Raw OHLCV comes from Alpha Vantage; technical columns are calculated deterministically from that same series."
                  />
        ...
        <footer className="rounded-2xl border border-white/10 bg-black/45 p-4 text-xs leading-6 text-slate-500">
          Forecasts are quantitative
          estimates derived from Alpha
          Vantage OHLCV and are not
          guarantees, trade instructions,
          or client recommendations.
          Fundamentals update when issuers
          report financials; news and
          options availability depend on
          the Alpha Vantage subscription
