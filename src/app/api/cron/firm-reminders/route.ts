...
function json(body: unknown, init?: ResponseInit) {
  ...
  response.headers.set("X-Slice-Cron-Route", "firm-reminders-v3");
  return response;
}
...
function parseReminderDate(reminderAt: string | null, baseDate: Date) {
  ...
  if (lower === "now" || lower.includes("asap")) {
...
function isCompleteStatus(status: string | null | undefined) {
...
async function processTimedReminders(limit: number) {
  ...
  for (const comment of comments) {
    ...
    if (task && isCompleteStatus(task.status)) {
      skippedComplete += 1;
      continue;
    }
    ...
    await prisma.notificationDelivery.create({
      data: {
        ...
        status: "Delivered",
        urgency: isHighPriority(task?.priority) ? "High" : "Medium",
        score: isHighPriority(task?.priority) ? 84 : 70,
        title,
        body: `${projectLine}${dueLine}${parsed.message}`,
        reason: `${reasonKey}; createdBy:${
          parsed.createdBy ?? comment.user.email
        }`,
        simulated: false,
  ...
  return {
    ...
    skippedTimedComplete: skippedComplete,
  ...
  for (const project of projects) {
    ...
    for (const target of targets) {
      const reasonKey = notificationDedupeReason(
        "project-deadline",
        project.id,
        `${project.dueDate}-${dueLabel}-${today}`
      );
...
async function processOpenTaskFollowups(limit: number) {
  const today = ymd(new Date());

  const tasks = await prisma.firmAgendaTask.findMany({
    where: {
      dueDate: {
        lte: today,
      },
  ...
  for (const task of tasks) {
    ...
    const target = task.agenda.membership;
    ...
    await prisma.notificationDelivery.create({
      data: {
        ...
        destination: target.user?.email ?? null,
        status: "Delivered",
        urgency: isOverdue || isHighPriority(task.priority) ? "High" : "Medium",
        score: isOverdue || isHighPriority(task.priority) ? 86 : 70,
        title: `${isOverdue ? "Overdue task" : "Task follow-up"}: ${
          task.title
        }`,
        body: `${task.project ? `Project: ${task.project.title}. ` : ""}Status: ${
          task.status
        }. Due: ${
          task.dueDate ?? "No due date"
        }. This reminder will continue until the task is marked complete.`,
        reason: reasonKey,
        simulated: false,
        deliveredAt: new Date(),
      },
  ...
  return {
    scannedOpenTaskFollowups: scanned,
    deliveredOpenTaskFollowups: delivered,
    skippedOpenTaskFollowupDuplicate: skippedDuplicate,
  };
}
...
export async function GET(request: Request) {
  ...
  const [timed, deadlines, followups] = await Promise.all([
    processTimedReminders(limit),
    processProjectDeadlines(limit),
    processOpenTaskFollowups(limit),
  ]);

  return json({
    ok: true,
    route: "/api/cron/firm-reminders",
    version: "firm-reminders-v3",
    ...
    totals: {
      delivered:
        ...
        followups.deliveredOpenTaskFollowups,
      duplicates:
        timed.skippedTimedDuplicate +
        deadlines.skippedProjectDuplicate +
        followups.skippedOpenTaskFollowupDuplicate,
    },
  });
}
