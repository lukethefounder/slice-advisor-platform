...
export async function GET(request: Request) {
  ...
  const checkpoint = await readPublicationCheckpoint();

  if (!force && checkpoint?.lastFetchedAt) {
    const lastPublication = easternClock(checkpoint.lastFetchedAt);
  ...
  try {
    ...
    if (selectedArticles.length < DAILY_ARTICLE_COUNT) {
      warnings.push(
    ...
    return json({
      ...
      articleLimit: DAILY_ARTICLE_COUNT,
      totalRankedArticleCount: snapshot.items.length,
      selectedArticleIds: selectedArticles.map(
        (article) => article.id,
      ),
