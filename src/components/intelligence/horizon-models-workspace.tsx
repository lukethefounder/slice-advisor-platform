"use client";

import Link from "next/link";
import {
  AlertTriangle,
  ArrowLeft,
  BarChart3,
  BrainCircuit,
  CheckCircle2,
  Clock3,
  Database,
  Gauge,
  Layers3,
  Play,
  RefreshCw,
  ShieldCheck,
  Target,
  TrendingUp,
} from "lucide-react";
import {
  useCallback,
  useEffect,
  useMemo,
  useRef,
  useState,
  type ChangeEvent,
} from "react";

import {
  IntelligenceMetric,
  IntelligenceNotice,
  IntelligencePage,
  IntelligencePill,
  IntelligenceSectionHeading,
  IntelligenceSurface,
  formatIntelligenceDate,
  formatIntelligenceInteger,
  formatIntelligenceNumber,
  formatIntelligencePercent,
} from "@/components/intelligence/intelligence-ui";
import {
  OperatingMemoryPills,
  type ClientOperatingMemory,
  statusTone,
  useVisibilityRefresh,
} from "@/components/intelligence/operating-memory-ui";
import {
  intelligenceFetch,
  isAbortError,
} from "@/lib/intelligence/client";

type TrainingMetrics = {
  sampleCount: number;
  brierScore: number;
  directionalAccuracyPercent: number;
  meanAbsoluteReturnError: number;
  averageForecastProbability: number;
  observedPositivePercent: number;
};

type HorizonArtifact = {
  horizon: string;
  label: string;
  status: "TRAINED" | "PRIOR";
  sampleCount: number;
  trainingCount: number;
  validationCount: number;
  trainedAt: string;
  returnScale: number;
  validationMetrics: TrainingMetrics;
};

type HorizonSuite = {
  schemaVersion: string;
  modelFamily: string;
  modelVersion: string;
  engineVersion: string;
  calibrationVersion: string;
  trainedAt: string;
  status: "TRAINED" | "PRIOR";
  totalEligibleSamples: number;
  artifacts: Record<string, HorizonArtifact>;
  safeguards: {
    autonomousTradingEnabled: false;
    automaticPromotionEnabled: false;
    shadowOnly: true;
  };
};

type HorizonEvaluation = {
  generatedAt: string;
  modelVersion: string | null;
  predictionCount: number;
  matchedOutcomeCount: number;
  overall: TrainingMetrics & {
    averageConfidence?: number;
  };
  byHorizon: Array<
    TrainingMetrics & {
      horizon: string;
      averageConfidence?: number;
    }
  >;
};

type ModelRecord = {
  id: string;
  displayName: string;
  modelVersion: string;
  engineVersion: string;
  calibrationVersion: string;
  status: string;
  createdAt: string;
  promotedAt: string | null;
  metadata?: Record<string, unknown>;
};

type HorizonResponse = {
  ok: true;
  generatedAt: string;
  activeSuite: HorizonSuite;
  models: ModelRecord[];
  evaluation: HorizonEvaluation;
  memory: ClientOperatingMemory;
  operatingMode: string;
  terminology: {
    shadow: string;
    prior: string;
  };
  safeguards: {
    autonomousTradingEnabled: false;
    automaticPromotionEnabled: false;
    activeMode: string;
    replacesProductionForecast: false;
  };
};

const HORIZON_ORDER = [
  "5-30m",
  "intraday",
  "1d",
  "2-5d",
  "1-4w",
  "1-3m",
  "3-12m",
  "1-3y",
] as const;

function metricTone(value: number, direction: "higher" | "lower") {
  const healthy =
    direction === "higher" ? value >= 52 : value > 0 && value <= 0.25;

  return healthy ? ("emerald" as const) : ("amber" as const);
}

export default function HorizonModelsWorkspace() {
  const [data, setData] = useState<HorizonResponse | null>(null);
  const [selectedRunId, setSelectedRunId] = useState("");
  const [loading, setLoading] = useState(true);
  const [operation, setOperation] = useState<"train" | "generate" | null>(
    null,
  );
  const [confirmTraining, setConfirmTraining] = useState(false);
  const [online, setOnline] = useState(true);
  const [message, setMessage] = useState(
    "Loading independent horizon-model operating state.",
  );
  const [error, setError] = useState("");
  const requestController = useRef<AbortController | null>(null);
  const mounted = useRef(true);

  const load = useCallback(async (quiet = false) => {
    requestController.current?.abort();
    const controller = new AbortController();
    requestController.current = controller;

    if (!quiet) {
      setLoading(true);
      setMessage("Loading model artifacts, settled outcomes, and memory.");
    }

    try {
      const response = await intelligenceFetch<HorizonResponse>(
        "/api/intelligence/forecast/horizon-models?days=30",
        {
          signal: controller.signal,
        },
        {
          timeoutMs: 35_000,
          retries: 1,
        },
      );

      if (!mounted.current) return;

      setData(response);
      setError("");
      setSelectedRunId((current) => {
        if (
          current &&
          response.memory.recentRuns.some((run) => run.id === current)
        ) {
          return current;
        }

        return response.memory.recentRuns[0]?.id ?? "";
      });
      setMessage(
        `${response.operatingMode} loaded with ${response.memory.summary.horizonPredictions.toLocaleString()} retained horizon predictions.`,
      );
    } catch (caught) {
      if (isAbortError(caught) || !mounted.current) return;

      setError(
        caught instanceof Error
          ? caught.message
          : "Unable to load horizon-model operations.",
      );
    } finally {
      if (mounted.current) setLoading(false);
    }
  }, []);

  useEffect(() => {
    mounted.current = true;
    setOnline(navigator.onLine);
    const updateOnline = () => setOnline(navigator.onLine);
    window.addEventListener("online", updateOnline);
    window.addEventListener("offline", updateOnline);
    void load();

    return () => {
      mounted.current = false;
      requestController.current?.abort();
      window.removeEventListener("online", updateOnline);
      window.removeEventListener("offline", updateOnline);
    };
  }, [load]);

  useVisibilityRefresh(
    () => load(true),
    {
      intervalMs: 60_000,
      enabled: true,
      busy: Boolean(operation) || loading,
    },
  );

  async function runAction(action: "train" | "generate") {
    if (action === "generate" && !selectedRunId) {
      setError("Select a retained forecast run first.");
      return;
    }

    setOperation(action);
    setError("");
    setMessage(
      action === "train"
        ? "Training independent horizon artifacts from eligible settled outcomes."
        : "Generating eight independent horizon predictions from the selected immutable forecast snapshot.",
    );

    try {
      const response = await intelligenceFetch<{
        ok: true;
        message?: string;
      }>(
        "/api/intelligence/forecast/horizon-models",
        {
          method: "POST",
          body: JSON.stringify(
            action === "train"
              ? {
                  action: "train",
                }
              : {
                  action: "generate-run",
                  runId: selectedRunId,
                },
          ),
        },
        {
          timeoutMs: 118_000,
        },
      );

      if (!mounted.current) return;

      setConfirmTraining(false);
      setMessage(
        response.message ??
          (action === "train"
            ? "Horizon-model training completed."
            : "Independent horizon predictions were retained."),
      );
      await load(true);
    } catch (caught) {
      if (isAbortError(caught) || !mounted.current) return;

      setError(
        caught instanceof Error
          ? caught.message
          : "The horizon-model operation did not complete.",
      );
    } finally {
      if (mounted.current) setOperation(null);
    }
  }

  const artifacts = useMemo(
    () =>
      HORIZON_ORDER.flatMap((horizon) => {
        const artifact = data?.activeSuite.artifacts[horizon];
        return artifact ? [artifact] : [];
      }),
    [data],
  );
  const trainedCount = artifacts.filter(
    (artifact) => artifact.status === "TRAINED",
  ).length;
  const selectedRun = data?.memory.recentRuns.find(
    (run) => run.id === selectedRunId,
  );
  const latestPrediction =
    data?.memory.horizonPredictions[0] ?? null;

  return (
    <IntelligencePage>
      <IntelligenceSurface className="overflow-hidden">
        <div className="bg-[radial-gradient(circle_at_8%_0%,rgba(124,58,237,0.10),transparent_34%),radial-gradient(circle_at_95%_4%,rgba(16,185,129,0.12),transparent_36%)] p-5 sm:p-7">
          <div className="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
            <div className="max-w-5xl">
              <OperatingMemoryPills
                memory={data?.memory ?? null}
                online={online}
                busy={Boolean(operation)}
              >
                {data ? (
                  <IntelligencePill tone={statusTone(data.activeSuite.status)}>
                    <Layers3 className="h-3.5 w-3.5" />
                    {data.activeSuite.status}
                  </IntelligencePill>
                ) : null}
              </OperatingMemoryPills>

              <h1 className="mt-5 text-4xl font-black tracking-[-0.05em] text-[var(--slice-heading)] sm:text-5xl xl:text-6xl">
                Independent models for every forecast horizon.
              </h1>
              <p className="mt-4 max-w-4xl text-sm font-semibold leading-7 text-[var(--slice-muted)] sm:text-base">
                Slice maintains eight distinct probability and expected-return
                models, evaluates them against settled non-demo outcomes, and
                retains model artifacts, predictions, and validation history
                in a durable 30-day operating-memory view.
              </p>
            </div>

            <Link
              href="/workspace/intelligence"
              prefetch={false}
              className="inline-flex min-h-11 shrink-0 items-center gap-2 rounded-xl border border-[var(--slice-border)] bg-[var(--slice-surface-strong)] px-4 text-xs font-black text-[var(--slice-text)] transition hover:border-[var(--slice-accent-border)]"
            >
              <ArrowLeft className="h-4 w-4" />
              Control plane
            </Link>
          </div>

          <div className="mt-7 grid gap-3 xl:grid-cols-[minmax(280px,1fr)_auto_auto_auto]">
            <label className="grid min-h-14 gap-1 rounded-2xl border border-[var(--slice-border)] bg-[var(--slice-surface-strong)] px-4 py-2.5 shadow-sm">
              <span className="text-[9px] font-black uppercase tracking-[0.12em] text-[var(--slice-subtle)]">
                Retained forecast snapshot
              </span>
              <select
                value={selectedRunId}
                onChange={(event: ChangeEvent<HTMLSelectElement>) =>
                  setSelectedRunId(event.target.value)
                }
                disabled={!data?.memory.recentRuns.length || Boolean(operation)}
                className="min-w-0 bg-transparent text-sm font-black text-[var(--slice-heading)] outline-none disabled:opacity-50"
              >
                {!data?.memory.recentRuns.length ? (
                  <option value="">No forecast runs in memory</option>
                ) : null}
                {data?.memory.recentRuns.map((run) => (
                  <option key={run.id} value={run.id}>
                    {run.symbol} · {formatIntelligenceDate(run.generatedAt)} ·{" "}
                    {run.status}
                  </option>
                ))}
              </select>
            </label>

            <button
              type="button"
              onClick={() => void load()}
              disabled={loading || Boolean(operation)}
              className="inline-flex min-h-14 items-center justify-center gap-2 rounded-2xl border border-[var(--slice-border)] bg-[var(--slice-surface-strong)] px-5 text-sm font-black text-[var(--slice-text)] shadow-sm transition hover:border-[var(--slice-accent-border)] disabled:opacity-50"
            >
              <RefreshCw
                className={`h-4 w-4 ${loading ? "animate-spin" : ""}`}
              />
              Refresh state
            </button>

            <button
              type="button"
              onClick={() => void runAction("generate")}
              disabled={
                !selectedRunId || loading || Boolean(operation) || !online
              }
              className="inline-flex min-h-14 items-center justify-center gap-2 rounded-2xl bg-[var(--slice-accent-strong)] px-5 text-sm font-black text-white shadow-[0_14px_32px_var(--slice-accent-glow)] disabled:opacity-50"
            >
              {operation === "generate" ? (
                <RefreshCw className="h-4 w-4 animate-spin" />
              ) : (
                <Play className="h-4 w-4 fill-current" />
              )}
              Generate horizon set
            </button>

            <button
              type="button"
              onClick={() => setConfirmTraining(true)}
              disabled={loading || Boolean(operation) || !online}
              className="inline-flex min-h-14 items-center justify-center gap-2 rounded-2xl border border-violet-600/20 bg-violet-50 px-5 text-sm font-black text-violet-800 transition hover:bg-violet-100 disabled:opacity-50 dark:border-violet-400/25 dark:bg-violet-500/10 dark:text-violet-100"
            >
              <BrainCircuit className="h-4 w-4" />
              Train models
            </button>
          </div>

          {selectedRun ? (
            <p className="mt-3 text-[10px] font-semibold text-[var(--slice-muted)]">
              Selected snapshot: {selectedRun.symbol}, provider as of{" "}
              {formatIntelligenceDate(selectedRun.asOfAt)}, quality{" "}
              {formatIntelligenceNumber(selectedRun.dataQualityScore, 0)}%.
            </p>
          ) : null}

          {confirmTraining ? (
            <IntelligenceNotice
              className="mt-4"
              tone="violet"
              icon={<ShieldCheck className="h-5 w-5" />}
            >
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <p className="font-black">Confirm governed model training</p>
                  <p className="mt-1 text-xs font-semibold leading-5">
                    Training uses settled, point-in-time-safe, non-demo outcomes.
                    The artifact is retained for evaluation and does not
                    automatically replace a production model.
                  </p>
                </div>
                <div className="flex shrink-0 gap-2">
                  <button
                    type="button"
                    onClick={() => setConfirmTraining(false)}
                    className="min-h-10 rounded-xl border border-current/20 px-3 text-[11px] font-black"
                  >
                    Cancel
                  </button>
                  <button
                    type="button"
                    onClick={() => void runAction("train")}
                    className="min-h-10 rounded-xl bg-violet-700 px-4 text-[11px] font-black text-white"
                  >
                    Train from settled outcomes
                  </button>
                </div>
              </div>
            </IntelligenceNotice>
          ) : null}

          <IntelligenceNotice
            className="mt-4"
            tone={error ? "rose" : data ? "emerald" : "slate"}
            icon={
              error ? (
                <AlertTriangle className="h-5 w-5" />
              ) : data ? (
                <CheckCircle2 className="h-5 w-5" />
              ) : (
                <RefreshCw className="h-5 w-5 animate-spin" />
              )
            }
          >
            {error || message}
          </IntelligenceNotice>

          {data && !data.memory.recentRuns.length ? (
            <IntelligenceNotice
              className="mt-4"
              tone="amber"
              icon={<Target className="h-4 w-4" />}
            >
              No retained forecast snapshot is available. Create a current,
              point-in-time forecast in the{" "}
              <Link
                href="/workspace/intelligence/forecast-lab"
                prefetch={false}
                className="font-black underline"
              >
                Forecast Lab
              </Link>{" "}
              before generating independent horizon predictions.
            </IntelligenceNotice>
          ) : null}
        </div>
      </IntelligenceSurface>

      {data ? (
        <>
          <section className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-7">
            <IntelligenceMetric
              label="Model status"
              value={data.activeSuite.status}
              helper={data.operatingMode}
              icon={<Layers3 className="h-5 w-5" />}
              tone={statusTone(data.activeSuite.status)}
            />
            <IntelligenceMetric
              label="Trained horizons"
              value={`${trainedCount} / ${artifacts.length || 8}`}
              helper="Independent horizon artifacts."
              icon={<Target className="h-5 w-5" />}
              tone={trainedCount === 8 ? "emerald" : "amber"}
            />
            <IntelligenceMetric
              label="Eligible samples"
              value={formatIntelligenceInteger(
                data.activeSuite.totalEligibleSamples,
              )}
              helper="Settled outcomes available to the suite."
              icon={<Database className="h-5 w-5" />}
              tone="cyan"
            />
            <IntelligenceMetric
              label="Predictions in memory"
              value={formatIntelligenceInteger(
                data.memory.summary.horizonPredictions,
              )}
              helper={data.memory.window.label}
              icon={<BarChart3 className="h-5 w-5" />}
              tone="violet"
            />
            <IntelligenceMetric
              label="Matched outcomes"
              value={formatIntelligenceInteger(
                data.evaluation.matchedOutcomeCount,
              )}
              helper="Stored predictions with settled results."
              icon={<CheckCircle2 className="h-5 w-5" />}
            />
            <IntelligenceMetric
              label="Directional accuracy"
              value={`${formatIntelligenceNumber(
                data.evaluation.overall.directionalAccuracyPercent,
                1,
              )}%`}
              helper="Across matched retained outcomes."
              icon={<TrendingUp className="h-5 w-5" />}
              tone={metricTone(
                data.evaluation.overall.directionalAccuracyPercent,
                "higher",
              )}
            />
            <IntelligenceMetric
              label="Brier score"
              value={formatIntelligenceNumber(
                data.evaluation.overall.brierScore,
                4,
              )}
              helper="Probability error; lower is better."
              icon={<Gauge className="h-5 w-5" />}
              tone={metricTone(
                data.evaluation.overall.brierScore,
                "lower",
              )}
            />
          </section>

          <IntelligenceSurface className="mt-5 p-5 sm:p-6">
            <IntelligenceSectionHeading
              eyebrow="Eight-horizon suite"
              title="Training and validation by time horizon"
              description="Each horizon keeps its own sample count, probability coefficients, expected-return scale, and chronological validation metrics."
              action={
                <IntelligencePill tone={statusTone(data.activeSuite.status)}>
                  <Clock3 className="h-3.5 w-3.5" />
                  Trained {formatIntelligenceDate(data.activeSuite.trainedAt)}
                </IntelligencePill>
              }
            />

            <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              {artifacts.map((artifact) => (
                <article
                  key={artifact.horizon}
                  className="rounded-2xl border border-[var(--slice-border)] bg-[var(--slice-surface-strong)] p-5 shadow-sm"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <IntelligencePill tone={statusTone(artifact.status)}>
                        {artifact.status}
                      </IntelligencePill>
                      <h3 className="mt-3 text-lg font-black text-[var(--slice-heading)]">
                        {artifact.label}
                      </h3>
                      <p className="mt-1 text-[10px] font-black uppercase tracking-[0.12em] text-[var(--slice-subtle)]">
                        {artifact.horizon}
                      </p>
                    </div>
                    <span className="text-2xl font-black text-[var(--slice-accent-strong)]">
                      {formatIntelligenceInteger(artifact.sampleCount)}
                    </span>
                  </div>

                  <div className="mt-4 grid grid-cols-2 gap-2">
                    {[
                      [
                        "Validation",
                        formatIntelligenceInteger(artifact.validationCount),
                      ],
                      [
                        "Brier",
                        formatIntelligenceNumber(
                          artifact.validationMetrics.brierScore,
                          4,
                        ),
                      ],
                      [
                        "Accuracy",
                        `${formatIntelligenceNumber(
                          artifact.validationMetrics
                            .directionalAccuracyPercent,
                          1,
                        )}%`,
                      ],
                      [
                        "Return MAE",
                        formatIntelligencePercent(
                          artifact.validationMetrics
                            .meanAbsoluteReturnError,
                          2,
                        ),
                      ],
                    ].map(([label, value]) => (
                      <div
                        key={label}
                        className="rounded-xl border border-[var(--slice-border)] bg-[var(--slice-surface-muted)] p-3"
                      >
                        <p className="text-[8px] font-black uppercase tracking-[0.1em] text-[var(--slice-subtle)]">
                          {label}
                        </p>
                        <p className="mt-1 text-sm font-black text-[var(--slice-heading)]">
                          {value}
                        </p>
                      </div>
                    ))}
                  </div>

                  <p className="mt-3 text-[10px] font-semibold leading-5 text-[var(--slice-muted)]">
                    Expected-return scale{" "}
                    {formatIntelligenceNumber(artifact.returnScale, 2)} ·{" "}
                    {artifact.trainingCount.toLocaleString()} training rows.
                  </p>
                </article>
              ))}
            </div>
          </IntelligenceSurface>

          <section className="mt-5 grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
            <IntelligenceSurface className="p-5 sm:p-6">
              <IntelligenceSectionHeading
                eyebrow="Model registry"
                title="Stored model artifacts"
                description="Production, candidate, and evaluation artifacts remain versioned and separately governed."
              />
              <div className="mt-5 space-y-3">
                {data.models.length ? (
                  data.models.map((model) => (
                    <article
                      key={model.id}
                      className="rounded-2xl border border-[var(--slice-border)] bg-[var(--slice-surface-strong)] p-4"
                    >
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            <IntelligencePill tone={statusTone(model.status)}>
                              {model.status}
                            </IntelligencePill>
                            <span className="text-[9px] font-black uppercase tracking-[0.1em] text-[var(--slice-subtle)]">
                              {formatIntelligenceDate(model.createdAt)}
                            </span>
                          </div>
                          <h3 className="mt-3 truncate text-sm font-black text-[var(--slice-heading)]">
                            {model.displayName}
                          </h3>
                          <p className="mt-1 break-all text-[10px] font-semibold text-[var(--slice-muted)]">
                            {model.modelVersion}
                          </p>
                        </div>
                        <div className="text-right text-[10px] font-semibold text-[var(--slice-muted)]">
                          <p>{model.engineVersion}</p>
                          <p className="mt-1">{model.calibrationVersion}</p>
                        </div>
                      </div>
                    </article>
                  ))
                ) : (
                  <IntelligenceNotice tone="amber">
                    No stored model artifact exists yet. Training will create
                    the first governed artifact.
                  </IntelligenceNotice>
                )}
              </div>
            </IntelligenceSurface>

            <IntelligenceSurface className="p-5 sm:p-6">
              <IntelligenceSectionHeading
                eyebrow="Operating memory"
                title="Recent independent predictions"
                description="Predictions are retained with forecast-run, model, horizon, probability, confidence, and creation time."
              />
              <div className="mt-5 space-y-3">
                {data.memory.horizonPredictions.slice(0, 10).map(
                  (prediction) => (
                    <article
                      key={prediction.id}
                      className="rounded-2xl border border-[var(--slice-border)] bg-[var(--slice-surface-strong)] p-4"
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="text-sm font-black text-[var(--slice-heading)]">
                            {prediction.symbol ?? "Forecast"} ·{" "}
                            {prediction.horizon ?? "Horizon"}
                          </p>
                          <p className="mt-1 text-[10px] font-semibold text-[var(--slice-muted)]">
                            {formatIntelligenceDate(prediction.createdAt)}
                          </p>
                        </div>
                        <IntelligencePill
                          tone={statusTone(
                            prediction.direction ?? prediction.status,
                          )}
                        >
                          {prediction.direction ?? prediction.status}
                        </IntelligencePill>
                      </div>
                      <div className="mt-3 flex flex-wrap gap-3 text-[10px] font-black text-[var(--slice-muted)]">
                        <span>
                          Probability{" "}
                          {formatIntelligenceNumber(
                            prediction.probability,
                            1,
                          )}
                          %
                        </span>
                        <span>
                          Return{" "}
                          {formatIntelligencePercent(
                            prediction.expectedReturnPercent,
                            2,
                          )}
                        </span>
                        <span>
                          Confidence{" "}
                          {formatIntelligenceNumber(
                            prediction.confidence,
                            0,
                          )}
                          %
                        </span>
                      </div>
                    </article>
                  ),
                )}

                {!data.memory.horizonPredictions.length ? (
                  <IntelligenceNotice tone="slate">
                    No independent predictions are retained in the current
                    operating-memory window. Select a forecast snapshot and
                    generate a horizon set.
                  </IntelligenceNotice>
                ) : null}
              </div>
            </IntelligenceSurface>
          </section>

          <IntelligenceSurface className="mt-5 p-5 sm:p-6">
            <IntelligenceSectionHeading
              eyebrow="Governed operation"
              title="How model status is used"
              description="Training, evaluation, and promotion are distinct operating states. No artifact promotes itself and no model places trades."
            />
            <div className="mt-5 grid gap-3 lg:grid-cols-2">
              <IntelligenceNotice
                tone="violet"
                icon={<BrainCircuit className="h-4 w-4" />}
              >
                {data.terminology.shadow}
              </IntelligenceNotice>
              <IntelligenceNotice
                tone="amber"
                icon={<ShieldCheck className="h-4 w-4" />}
              >
                {data.terminology.prior}
              </IntelligenceNotice>
            </div>
          </IntelligenceSurface>
        </>
      ) : null}

      {loading && !data ? (
        <IntelligenceSurface className="mt-5 p-8">
          <div className="grid min-h-64 place-items-center text-center">
            <div>
              <RefreshCw className="mx-auto h-6 w-6 animate-spin text-[var(--slice-accent-strong)]" />
              <p className="mt-3 text-sm font-black text-[var(--slice-heading)]">
                Loading horizon-model operations
              </p>
            </div>
          </div>
        </IntelligenceSurface>
      ) : null}

      {latestPrediction ? (
        <p className="mt-4 text-center text-[10px] font-semibold text-[var(--slice-subtle)]">
          Latest retained prediction:{" "}
          {formatIntelligenceDate(latestPrediction.createdAt)}.
        </p>
      ) : null}
    </IntelligencePage>
  );
}